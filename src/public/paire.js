...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (params.has('ref')) {
        ...
        // Enregistre le parrainage côté serveur si le filleul renseigne son numéro
        const phoneInput = document.getElementById('phoneNumber');
        phoneInput.addEventListener('blur', async function() {
            const filleul = phoneInput.value.trim();
            if (validatePhoneNumber(filleul) && inviterPhone !== filleul) {
                await fetch('/api/parrainage', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ parrain: inviterPhone, filleul })
                });
            }
        });
    }
